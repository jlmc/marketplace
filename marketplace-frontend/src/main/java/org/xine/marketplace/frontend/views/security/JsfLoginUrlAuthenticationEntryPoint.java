 ...
 *         Time: 6:17 PM
 */
public class JsfLoginUrlAuthenticationEntryPoint implements AuthenticationEntryPoint,
InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        Assert.isTrue(
                StringUtils.hasText(this.loginFormUrl)
                && UrlUtils.isValidRedirectUrl(this.loginFormUrl),
    ...
    /**
     * Allows subclasses to modify the login form URL that should be applicable for a given
     * request.
     * @param request
     *            the request
     * @param response
    ...
    public void commence(final HttpServletRequest request, final HttpServletResponse response,
        ...
        if (this.useForward) {
            ...
            if (redirectUrl == null) {
                final String loginForm = determineUrlToUseForThisRequest(request, response,
                        authException);
    ...
    /**
     * Set to true to force login form access to be via https. If this value is true (the default
     * is false),
     * and the incoming request for the protected resource which triggered the interceptor was not
     * already <code>https</code>, then the client will first be redirected to an https URL, even
     * if <tt>serverSideRedirect</tt> is set to <tt>true</tt>.
     */
    public void setForceHttps(final boolean forceHttps) {
     ...
     * @param useForward
     *            true if a forward to the login page should be used. Must be false (the default)
     *            if {@code loginFormUrl} is set to an absolute value.
     */
